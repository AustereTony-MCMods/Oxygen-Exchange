...
public class ContainerExchangeMenu extends Container {

    private final EntityPlayer player;

    private final IInventory playerInventory, exchangeInventory;

    public ContainerExchangeMenu(EntityPlayer player, IInventory playerInventory) {
        this.player = player;
        this.playerInventory = playerInventory; 
        this.exchangeInventory = new InventoryExchangeMenu();
        int j;                  
        for (j = 0; j < 5; j++)//'preview' slots (other player offer)   
            this.addSlotToContainer(new SlotExchange(this.exchangeInventory, j, 0, 0, true));
        for (j = 5; j < 10; j++)//client offer slots      
            this.addSlotToContainer(new SlotExchange(this.exchangeInventory, j, 0, 0));
    ...
    public void disableOfferSlots() {
        for (int i = 5; i < 10; i++)
            ((SlotExchange) this.inventorySlots.get(i)).setDisabled(true);
    }

    public void enableOfferSlots() {
        for (int i = 5; i < 10; i++)
            ((SlotExchange) this.inventorySlots.get(i)).setDisabled(false);
    }
}
